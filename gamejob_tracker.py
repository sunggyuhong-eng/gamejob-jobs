...
def crawl_all_listings(max_pages=200, sleep_sec=0.5, verbose=True):
    ...
    for page in range(1, hard_stop_page + 1):
        ...
        try:
            ...
            print(f"[crawl] 요청 실패: page {page}: {exc}")
    ...
    if failed_pages:
        raise RuntimeError(f"수집 요청 실패 페이지 {failed_pages}: 불완전한 스냅샷은 저장하지 않습니다.")
    if not total_count:
        raise RuntimeError("전체 공고 수를 확인할 수 없어 수집 완전성을 검증하지 못했습니다. 저장을 보류합니다.")
    if len(all_jobs) < total_count:
        raise RuntimeError(f"수집 미완료: {len(all_jobs)}/{total_count}. 저장을 보류합니다.")
...
def save_snapshot(jobs, run_date: str, started_at=None) -> Path:
    path = SNAPSHOT_DIR / f"{run_date}.csv"
    if path.exists():
        raise RuntimeError(f"기존 스냅샷을 덮어쓰지 않습니다: {path}")
    ...
    path.with_suffix(".meta.json").write_text(json.dumps({
        "date": run_date, "status": "validated", "started_at": started_at,
        "finished_at": finished_at, "count": len(jobs),
        "schedule": "매월 1일 09:00 Asia/Seoul"
...
def load_latest_previous_snapshot(before_date: str):
    ...
    files = [f for f in files if f.stem[:7] < before_date[:7]]
    if not files:
        return None
    latest_month = files[-1].stem[:7]
    latest = next(f for f in files if f.stem[:7] == latest_month)
...
def generate_report(jobs, run_date: str):
    ...
    if prev:
        ...
        lines.append(f"- 신규 확인 공고: **{len(new_ids)}건**\n")
        lines.append(f"- 소멸(이전 대비 미노출) 공고: **{len(removed_ids)}건**\n")
        ...
        for kind in ("company", "category"):
            def keys(job):
                if kind == "company":
                    return [job.get("company_name") or "미분류"]
                categories = job.get("job_categories") or [job.get("job_function") or "미분류"]
                return list(set(categories.split(";") if isinstance(categories, str) else categories))
            def counts(rows):
                return Counter(key for row in rows for key in keys(row) if key)
            old, current = counts(prev_rows), counts(jobs)
            added, removed = counts(new_jobs), counts(removed_jobs)
            with (REPORT_DIR / f"{run_date}_{kind}_changes.csv").open("w", newline="", encoding="utf-8-sig") as f:
                writer = csv.writer(f)
                writer.writerow(["name", "previous", "current", "new", "removed", "net_change"])
                for name in sorted(set(old) | set(current), key=lambda name: current[name]-old[name], reverse=True):
                    writer.writerow([name, old[name], current[name], added[name], removed[name], current[name]-old[name]])
...
def main():
    ...
    if args.run:
        ...
        existing = sorted(SNAPSHOT_DIR.glob(f"{run_date[:7]}-*.csv"))
        if existing:
            print(f"[run] 해당 월 기준 자료가 이미 있습니다: {existing[0].name}. 원본을 유지합니다.")
            return
        ...
        if not jobs:
            ...
            raise RuntimeError("공고 수집 실패: 결과가 비어 있습니다.")
        ...
        save_snapshot(jobs, run_date, started_at.isoformat())
