...
LOGO_CACHE_PATH = DATA_DIR / "company_logos_cache.csv"
...
def load_logo_cache() -> dict:
    """company_id -> logo_url 캐시를 읽어옴."""
    cache = {}
    if LOGO_CACHE_PATH.exists():
        with LOGO_CACHE_PATH.open(encoding="utf-8-sig") as f:
            for row in csv.DictReader(f):
                cache[row["company_id"]] = row["logo_url"]
    return cache


def save_logo_cache(cache: dict):
    with LOGO_CACHE_PATH.open("w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(["company_id", "logo_url"])
        for company_id, logo_url in cache.items():
            w.writerow([company_id, logo_url])
...
def fetch_company_logo(session: requests.Session, company_id: str):
    """회사 상세페이지(Company/Detail)에서 로고 이미지 URL을 가져옴. 실패하면 None.
    이 페이지는 회사당 한 번만 있으므로, 공고 상세페이지보다 훨씬 확실하고 빠르게
    전체 회사의 로고를 채울 수 있습니다.

    태그 속성(src/data-src 등)에 의존하지 않고, 응답 본문 전체에서 로고 URL
    패턴을 정규식으로 직접 찾습니다 — 지연 로딩(lazy-load) 등으로 실제 주소가
    src가 아닌 다른 속성에 들어있어도 잡아낼 수 있게 하기 위함입니다.
    """
    try:
...
def backfill_company_logos(jobs, verbose=True):
    """jobs에 등장하는 모든 회사(company_id)에 대해 로고를 확보.
    ...
    missing = [cid for cid in company_ids if not logo_cache.get(cid)]

    if verbose:
        print(f"[logo] 전체 회사 수: {len(company_ids)}곳 / 로고 없는 회사: {len(missing)}곳")

    for i, company_id in enumerate(missing, 1):
        logo_url = fetch_company_logo(session, company_id)
        if logo_url:
            logo_cache[company_id] = logo_url
    ...
    for j in jobs:
        ...
        if cid and logo_cache.get(cid):
            j["company_logo_url"] = logo_cache[cid]
...
def enrich_with_categories(jobs, verbose=True):
    """jobs 리스트에 정확한 job_categories(공식 분류)와 회사 로고 URL을 채워넣음.
    ...
    gi_no_to_company_id = {j["gi_no"]: j.get("company_id") for j in jobs}

    for i, gi_no in enumerate(to_fetch, 1):
        cats, logo_url = fetch_job_detail_info(session, gi_no)
        if cats is not None:
            cat_cache[gi_no] = ";".join(cats)
        company_id = gi_no_to_company_id.get(gi_no)
        if logo_url and company_id:
            logo_cache[company_id] = logo_url
    ...
    for j in jobs:
        ...
        j["company_logo_url"] = logo_cache.get(j.get("company_id"), "")
