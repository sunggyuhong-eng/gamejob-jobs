...
def fetch_job_detail_info(session: requests.Session, gi_no: str):
    """모바일 상세페이지에서 '모집분야'(직종)와 회사 로고 URL을 함께 긁어옴.
    반환값: (categories_list_or_None, logo_url_or_None)"""
    try:
        ...
        logo_url = None
        logo_img = soup.find("img", src=lambda s: s and "CoImage/LogoView" in s)
        if logo_img and logo_img.get("src"):
            logo_url = logo_img["src"].strip()
