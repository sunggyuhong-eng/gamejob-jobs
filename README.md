...
- 직전 동기화 이후 새로 생긴 공고에만 `NEW` 표시하고 다음 동기화에서 자동 해제
...
사이트에 접속하거나 새로고침하면 Apps Script의 읽기 전용 API에서 최신 데이터를 자동으로 받아 표시합니다. 상단의 `데이터 다시 불러오기`도 Cloudflare나 GitHub Actions를 거치지 않고 같은 방식으로 현재 화면을 즉시 갱신합니다. 정식 배포 데이터는 기존대로 매시간 7분 GitHub Actions가 갱신하며, Apps Script를 일시적으로 불러오지 못할 때만 마지막 배포 데이터를 대신 표시합니다.
...
지원자 시트에서 필요한 헤더는 `진행단계`, `이름`, `직무(공고명)`입니다. `PJ` 열이 있으면 프로젝트명으로 사용하고, `입사 확정 날짜` 열이 있으면 Offer 지원자의 입사 예정일로 표시합니다.

원본의 B·D·F·H·X열을 지원자 사본의 A:E열에 행이 어긋나지 않게 가져오려면 `Dashboard_지원자`의 기존 수식을 아래처럼 교체합니다.

```text
=CHOOSECOLS(IMPORTRANGE("원본 스프레드시트 URL","'1. 2026 Interviewee'!A:X"),2,4,6,8,24)
```

이 결과는 `진행단계 | 이름 | PJ | 직무(공고명) | 입사 확정 날짜` 순서가 됩니다. 처음 연결할 때 `#REF!`가 표시되면 `액세스 허용`을 누릅니다.
